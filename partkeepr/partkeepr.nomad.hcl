job "$service_name" {
  ...
  group "oauth-$service_name" {
    ...
    task "oauth-$service_name" {
      driver = "docker"

      config {
        image = "$docker_image_oauth"
        ...
        labels {
          gelf_service_name="oauth-$service_name"
        }
      ...
      template {
        data = <<EOH
        PARTKEEPR_DATABASE_HOST="{{ key "oauth_$service_name/partkeepr_database_host" }}"
        PARTKEEPR_DATABASE_NAME="{{ key "oauth_$service_name/partkeepr_database_name" }}"
        PARTKEEPR_DATABASE_PORT="{{ key "oauth_$service_name/partkeepr_database_port" }}"
        PARTKEEPR_DATABASE_USER="{{ key "oauth_$service_name/partkeepr_database_user" }}"
        PARTKEEPR_DATABASE_PASS="{{ key "oauth_$service_name/partkeepr_database_pass" }}"
        PARTKEEPR_OKTOPART_APIKEY="{{ key "oauth_$service_name/partkeepr_oktopart_apikey" }}"
        EOH
        destination = "secrets/.env"
        env = true
        splay = "5s"
